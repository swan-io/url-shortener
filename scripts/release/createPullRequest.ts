...
const isGitRepoDirty = () =>
  Promise.all([
    ...
    isExecOk(
      "nbr=$(git ls-files --other --exclude-standard | wc -l); [ $nbr -gt 0 ]",
    ),
    isExecOk(
      'nbr=$(git ls-files -v | grep "^S" | cut -c3- | wc -l); test $nbr -eq 0',
    ),
  ]).then(
    ([isIndexClean, hasUnstagedChanges, hasUntrackedFiles, isSkipped]) => {
...
const createGhCompareUrl = (from: string | undefined, to: string) =>
  `https://github.com/swan-io/url-shortener/compare/${from != null ? `${from}..${to}` : ""}`;
...
(async () => {
  ...
  if (
    (await getLastGitCommitHash("main")) !==
    (await getLastGitCommitHash("origin/main"))
  ) {
  ...
  console.log(
    `🚀 Let's release ${pkg.name} (currently at ${currentPkgVersion.raw})`,
  );
